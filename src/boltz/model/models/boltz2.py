...
class Boltz2(LightningModule):
    ...
    def forward(
        ...
        for feat in feats.keys():
            if not isinstance(feats[feat], torch.Tensor):
                print(
                    "Non-tensor feature detected in feats:",
                    feat,
                    type(feats[feat]),
                    len(feats[feat]),
                    type(feats[feat][0]),
        ...
        with torch.set_grad_enabled(
            ...
            # FIXME: This is where it finds the contact for ligand
            # print(f"FEATS: {feats}")
            ...
            if self.run_trunk_and_structure:
                for i in range(recycling_steps + 1):
                    with torch.set_grad_enabled(
                        ...
                        # Apply recycling
                        # FIXME: The other place the ligand
