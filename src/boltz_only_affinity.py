...
from src.boltz.data.mol import load_canonicals
from src.boltz.data.msa.mmseqs2 import run_mmseqs2
from src.boltz.data.parse.a3m import parse_a3m
from src.boltz.data.parse.csv import parse_csv
from src.boltz.data.parse.fasta import parse_fasta
from src.boltz.data.parse.yaml import parse_yaml
from src.boltz.data.types import MSA, Manifest, Record
...
class TrainerAndArgs(NamedTuple):
    trainer: Trainer
    pairformer_args: PairformerArgs | PairformerArgsV2
    msa_args: MSAModuleArgs
    diffusion_params: BoltzDiffusionParams | Boltz2DiffusionParams

def setup_trainer_and_model(
    *,
    devices: int,
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    out_dir, cache, data = setup_io(
        data=data, cache=cache, out_dir=out_dir, model=model
    )
    ...
    loaded_setup = load_model_setup(
        out_dir=out_dir,
        override=override,
    )

    # Set up trainer
    trainer_and_args = setup_trainer_and_model(
        devices=devices,
        filtered_manifest=loaded_setup.filtered_manifest,
        model=model,
        step_scale=step_scale,
        subsample_msa=subsample_msa,
        num_subsampled_msa=num_subsampled_msa,
        out_dir=out_dir,
        processed=loaded_setup.processed,
        output_format=output_format,
        accelerator=accelerator,
    )
    ...
    print("Running affinity prediction")
    affinity_prediction(
        trainer_and_args=trainer_and_args,
        manifest=loaded_setup.manifest,
        out_dir=out_dir,
        processed=loaded_setup.processed,
        cache_dir=cache,
...
def affinity_prediction(
    *,
    trainer_and_args: TrainerAndArgs,
    manifest: Manifest,
    out_dir: Path,
    processed: BoltzProcessedInput,
    cache_dir: Path,
    ...
    """
    Runs affinity prediction on the provided manifest.

    Requires:

    - pre_affinity_affinity.npz from structure prediction
    ...
    if any(r.affinity for r in manifest.records):
        ...
        print(
            "predict_affinify", predict_affinity_args
        )
        print(
            "diffusion_params", asdict(trainer_and_args.diffusion_params)
        )
        print(
            "pairformer_args", asdict(trainer_and_args.pairformer_args)
        )
        print(
            "msa_args", asdict(trainer_and_args.msa_args)
        )
        ...
        model_module = Boltz2.load_from_checkpoint(
            ...
            map_location="cpu",
            diffusion_process_args=asdict(trainer_and_args.diffusion_params),
            ema=False,
            pairformer_args=asdict(trainer_and_args.pairformer_args),
            msa_args=asdict(trainer_and_args.msa_args),
        ...
        trainer_and_args.trainer.callbacks[0] = pred_writer
        trainer_and_args.trainer.predict(
            model_module,
            datamodule=data_module,
            return_predictions=False,
        )
...
if __name__ == "__main__":
    EXAMPLE_TARGET_DIR = (
        Path(__file__).parent.parent / "output/boltz_only_affinity"
    )
